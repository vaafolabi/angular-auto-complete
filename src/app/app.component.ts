...
export class AppComponent {
  ...
  if(searchStr.length <3) return;
     this.getFilteredDataFromWs(searchStr).subscribe((resp)=>{
      console.log(resp);
     this.options = resp.filter(s => s.name.includes(searchStr));
    });;

  }

  getFilteredDataFromWs(data):Observable<any[]>
  {
  ...
  getFilteredData = function (data) {
    ...
    return countries.filter(s => s.includes(data));
